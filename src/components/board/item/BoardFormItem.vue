...
if (props.type === "modify") {
  ...
  getModifyArticle(
    articleno,
    ({ data }) => {
      console.log("detailArticle  성공 데이터 : " ,data);
      // article.value.articleNo = data.articleNo;
      // article.value.subject = data.subject;
      // article.value.content = data.articleNo;
      article.value = data;
...
function moveDetail() {
  router.push({ name: "article-view", params:{articleno: article.value.articleNo} });
}
function moveList() {
